...
- ActiveRecordパターンでの開発経験者向け
  - アンチパターンに陥らない為の前提知識を得ること
- クリーンアーキテクチャに興味がある人向け
  - 採用のメリットや概念が理解できること
...
モジュール、クラスまたは関数が、単一の機能について責任を持ち、その機能をカプセル化するべきであるという原則  
この原則を守ることで、ソフトウェアの変更やテストが容易になり、コードの可読性や再利用性が向上する
...
この原則を守ることで、ソースコードの修正をせず、各要素の振る舞いを拡張することが可能になり、保守性や再利用性が向上する
...
この原則を守ることで、型の互換性や拡張性が保たれ、コードの再利用性や可読性が向上する
...
この原則を守ることで、クライアントに不要なメソッドへの依存を強制しないことができ、コードの可読性や保守性が向上する
...
この原則を守ることで、プログラムの重要な部分が、重要でない部分に依存しないように設計できます。  
これにより、コードの柔軟性や再利用性が向上します
...
モジュールは抽象に依存するようにし、これにより、内側のレイヤーは外側のレイヤーの変更に影響されなくなります<br />
Repositoryパターンによってドメインは永続化技術の詳細から隔離されます
</div>
...
* ActiveRecordパターンの場合(DRY)
  - データベースのテーブルとオブジェクトの構造がほぼ同じ
  - データベースの操作が単純
  - ビジネスロジックが複雑でない
* クリーンアーキテクチャの場合(SOLID)
  - ビジネスロジックやドメイン知識が重要
